"""KAN (Kolmogorov-Arnold Network) model with custom PyTorch training loop.

Architecture: Liu et al., "KAN: Kolmogorov-Arnold Networks" (arXiv 2404.19756,
ICLR 2025 oral). Backbone: vendored efficient-kan (see efficient_kan.py for
provenance). Learnable univariate B-spline activations on edges; nodes sum.

Two deliberate adaptations for tabular benchmarking, both documented in the
dissertation (docs/memo-novos-modelos-kan-tabkan-tabfm.md):
  * LayerNorm before every KAN layer (FastKAN-style): B-splines are only
    defined inside their grid; with standardized inputs a large share of
    values would otherwise fall outside it and silently degrade to the SiLU
    base branch alone.
  * grid_range=[-2, 2] (not the paper's [-1, 1]): after LayerNorm, ~95% of
    the mass lands inside the grid while keeping spline resolution where the
    data actually lives; the tails are handled by the SiLU base branch.
Training protocol matches the other DL models (AdamW, batch 256, early
stopping patience 20) rather than the paper's full-batch LBFGS — a declared
deviation adopted for benchmark comparability and stability.
"""
...
from src.models.efficient_kan import KANLinear
...
class _KANNet(nn.Module):
    ...
    def regularization_loss(self):
        return sum(layer.regularization_loss() for layer in self.kan_layers)
...
class KANModel(BaseModel):
    ...
    def __init__(self, task_type: str, n_classes: int | None = None, seed: int = 42, **kwargs):
        ...
        self.reg_lambda = kwargs.pop("reg_lambda", 0.0)
        ...
        self.spline_order = kwargs.pop("spline_order", 3)
        ...
        for epoch in range(self.max_epochs):
            ...
            for batch in train_loader:
                ...
                if self.reg_lambda > 0:
                    loss = loss + self.reg_lambda * self.model.regularization_loss()
